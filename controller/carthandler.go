...
func AddBook2Cart(w http.ResponseWriter, r *http.Request) {
	...
	if flag {
		...
		if cart != nil {
			//当前用户已经有购物车，此时需要判断购物车中是否有当前这本服装
			carItem, _ := dao.GetCartItemByBookIDAndCartID(bookID, cart.CartID)
			if carItem != nil {
				//购物车的购物项中已经有该服装，只需要将该服装所对应的购物项中的数量加1即可
				//1.获取购物车切片中的所有的购物项
				cts := cart.CartItems
				//2.遍历得到每一个购物项
				for _, v := range cts {
				...
				//购物车的购物项中还没有该服装，此时需要创建一个购物项并添加到数据库中
				//创建购物车中的购物项
			...
			//不管之前购物车中是否有当前服装对应的购物项，都需要更新购物车中的服装的总数量和总金额
			dao.UpdateCart(cart)
		} else {
...
func UpdateCartItem(w http.ResponseWriter, r *http.Request) {
	...
	for _, v := range cartItems {
		...
		if v.CartItemID == iCartItemID {
			//这个就是我们要更新的购物项
			//将当前购物项中的服装的数量设置为用户输入的值
			v.Count = iBookCount
			//更新数据库中该购物项的服装的数量和金额小计
			dao.UpdateBookCount(v)
		}
	}
	//更新购物车中的服装的总数量和总金额
